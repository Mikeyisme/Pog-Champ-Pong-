...
def draw_objects():
    pygame.draw.rect(screen, RED, (int(p1_x_pos), int(
        p1_y_pos), paddle_width, paddle_height))
    pygame.draw.rect(screen, RED, (int(p2_x_pos), int(
        p2_y_pos), paddle_width, paddle_height))
    pygame.draw.circle(screen, RED, (ball_x_pos, ball_y_pos), ball_width)
    score = game_font.render(
        f"{str(p1_score)} - {str(p2_score)}", False, RED)
...
pygame.display.set_caption("Pog Champ Pong!")
screen.fill(GREEN)
pygame.display.flip()
